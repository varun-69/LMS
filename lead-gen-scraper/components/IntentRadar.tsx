...
export default function IntentRadar({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Reddit posts (the free automated feed) */}
      <div>
        <h3 className="mb-2 text-sm font-semibold text-slate-700">
          Reddit — live buying-intent posts {posts.length ? `(${posts.length})` : ""}
        </h3>
        <div className="space-y-2">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    {p.channel} · u/{p.author} · {timeAgo(p.createdUtc)}
                  </p>
                </div>
                <button
                  onClick={() =>
                    onDraft({
                      targetName: `u/${p.author} on ${p.channel}`,
          ...
          {ran && posts.length === 0 && (
            <p className="rounded-lg border border-dashed border-slate-200 bg-white px-3 py-6 text-center text-sm text-slate-400">
              No Reddit posts in this window. Try a wider timeframe or the search links below.
            </p>
          )}
        </div>
      </div>
